...
int main(int argc, char **argv) {
  ...
  if (argc != 3) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "usage: calculate_joints '{rrbot_pose: {position: {x: X, y: Y, "
                "z: Z}}}'");
    return 1;
  }
  ...
  std::shared_ptr<rclcpp::Node> node =
      rclcpp::Node::make_shared("calculate_joints_client");
  rclcpp::Client<rrbot_gazebo::srv::CalculateJoints>::SharedPtr client =
      node->create_client<rrbot_gazebo::srv::CalculateJoints>(
          "calculate_joints");

  // Get the Pose position components of the request
  auto request =
      std::make_shared<rrbot_gazebo::srv::CalculateJoints::Request>();
  ...
  // Wait for service, exit if interrupted, wait if not available and print a
  // helpful message
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Joints: %f %f %f",
                result.get()->q0, result.get()->q1, result.get()->q2);
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
                 "Failed to call service calculate_joints");
  }
