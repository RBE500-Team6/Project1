...
class pd_controller_client(Node):
    ...
    def joints_within_limits(self):
        q1_within_limits = (
            (self.q_measured[0] < self.request.q_ref[0] + self.joint_limit) and
            (self.q_measured[0] > self.request.q_ref[0] - self.joint_limit))
        q2_within_limits = (
            (self.q_measured[1] < self.request.q_ref[1] + self.joint_limit) and
            (self.q_measured[1] > self.request.q_ref[1] - self.joint_limit))
        q3_within_limits = (
            (self.q_measured[2] < self.request.q_ref[2] + self.joint_limit) and
            (self.q_measured[2] > self.request.q_ref[2] - self.joint_limit))
...
def main(args=None):
    ...
    pd_controller_client = pd_controller_client()
    pd_controller_client.q_ref = int(sys.argv[1])
    # response = pd_controller_client.send_request(int(sys.argv[1]))

    rclpy.spin(pd_controller_client)

    pd_controller_client.destroy_node()
    rclpy.shutdown()
