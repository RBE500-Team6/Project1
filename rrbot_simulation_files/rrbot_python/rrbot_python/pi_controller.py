...
class pi_controller_pub(Node):
    ...
    def __init__(self):
        ...
        self.ts = 0.1 #10ms
        self.timer = self.create_timer(self.ts,
    ...
    def publish_effort_callback(self):
        ...
        if not self.waiting_for_subscriber():
            if not self.joints_within_limits():
                ...
                result = self.pi_client.send_request(twist1,twist2,twist3,twist4,twist5,twist6)
    ...
    def joints_within_limits(self):
        ...
        q1_within_limits = (abs(qvr[0] - self.joint_limit) <= abs(
                            qvm[0]) <=
                            abs(qvr[0] + self.joint_limit))
        q2_within_limits = (abs(qvr[1] - self.joint_limit) <= abs(
                            qvm[1]) <=
                            abs(qvr[1] + self.joint_limit))
        q3_within_limits = (abs(qvr[2] - self.joint_limit) <= abs(
                            qvm[2]) <=
...
def main(args=None):
    ...
    pi_controller.get_logger().info(
        ...
        (pi_controller.twist.data[0], pi_controller.twist.data[1],
         pi_controller.twist.data[2], pi_controller.twist.data[3], pi_controller.twist.data[4],
         pi_controller.twist.data[5]))
